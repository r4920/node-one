...
async function seedUser () {
  try {
    ...
    if (!user || !user.isPasswordMatch('BhVr5vrn5FAhWQ9') ) {
      let user = new User({
        'password':'BhVr5vrn5FAhWQ9',
        'username':'Billie86',
    ...
    let admin = await User.findOne({
      'username':'Adella.Braun84',
      'isDeleted':false,
      'isActive':true
    });
    if (!admin || !admin.isPasswordMatch('ouhE3XnsnlISV82') ) {
      let admin = new User({
        'password':'ouhE3XnsnlISV82',
        'username':'Adella.Braun84',
...
async function seedUserRole (){
  try {
    ...
    if (user && user.isPasswordMatch('BhVr5vrn5FAhWQ9') && userRole){
    ...
    if (admin && admin.isPasswordMatch('ouhE3XnsnlISV82') && adminRole){
      let count = await UserRole.countDocuments({
        userId: admin.id,
        roleId: adminRole.id
      });
